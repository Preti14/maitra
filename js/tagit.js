...
(function ($) {
	...
	//if($('#demo2').find('li.tagit-choice').text() =='')
    ...
    $.widget("ui.tagit", {
        ...
        options:{
            ...
            triggerKeys:['enter','telda'], //['enter', 'space', 'comma', 'tab'],
        ...
        _create:function () {
            ...
            if ($.inArray('space', self.options.triggerKeys) > 0 && $.inArray('comma', self.options.triggerKeys) > 0)
                ...
                self._splitAt = /,/g;
			else if ($.inArray('telda', self.options.triggerKeys) > 0)
                self._splitAt = /~/g;
            ...
            $(this.element).click(function (e) { 
                ...
                if ($(e.target).hasClass('tagit-close')) { 
                    ...
                    tag.element.remove();					
					$("#"+this.id).find("input:first").focus();
					
					
					if($(this).attr('id') == 'demo4') 
					var placeholder = 'From';
					if($(this).attr('id') == 'demo2') 
					var placeholder = 'To';
					if($(this).attr('id') == 'demo3') 
					var placeholder = 'Cc';
					if($(this).find('li.tagit-choice').length == 0){
						$(this).find('li.tagit-new input:text').attr('placeholder', placeholder);
					}
                    self._popTag(tag);			
            ...
            this.input.keydown(function (e) { 
				...
				//if( (!e.ctrlKey && e.keyCode==13) || e.keyCode==32 ){
				...
				if(e.ctrlKey && e.keyCode==13){
					...
					if(self.options.maxTags==1 && self.tag().index > 0)
						return false;
						...
						var mtype = $("#type").val();
						var mtype_input = $('#'+mtype+" :input").val();
						$("#title").val(mtype_input);
						$(".tagit-input").val("");				
						return false;
				}
        ...
        _popTag:function (tag) { 
			...
			if(tag.element.parent().attr('id') == 'demo4') 
			var placeholder = 'From';
			if(tag.element.parent().attr('id') == 'demo2') 
			var placeholder = 'To';
			if(tag.element.parent().attr('id') == 'demo3') 
			var placeholder = 'Cc';
			if(tag.element.parent().find('li.tagit-choice').length == 1){
				tag.element.parent().find('li.tagit-new input:text').attr('placeholder', placeholder);
			}
			var tv = tag.element.text();
			var tv_substr = tv.substr(0, tv.length-1);
			$("span:contains("+tv_substr+")").closest('tr').remove(); 
        ...
        _addTag:function (label, value) {
            ...
            tag.element = $('<li style="z-index:1" class="tagit-choice"'
			...
			if(type_input=="demo2"){
				$("#to_attention_table").append('<tr><td><span name="'+label+'">'+value+'</span></td><td>For <input type="text" class="form-control" /></td></tr>');
				//$("#to_attention_table").toggle();
			}
			if(type_input=="demo3"){
				$("#cc_attention_table").append('<tr><td><span name="'+label+'">'+value+'</span></td><td>For <input type="text" class="form-control" /></td></tr>');
			}
