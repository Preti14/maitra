...
class Home extends MY_Controller {
		...
		if($_POST){
			...
			if($this->form_validation->run() == true){
				...
				if($data['remember_me'] == "on") {    // if user check the remember me checkbox        
					setcookie('remember_me', $data['username'], time()+60*60*24*100, "/");
	...
	{ 
		...
		$this->distributed_list = $this->inm->distributed_list($v,$config["per_page"], $page);	
	...
	{   //print_r($_GET);
		...
		$this->load->model('inmails','inm');
		$config1['total_rows']    = $this->inm->division_list_rows($div_data['division_id'],$div_data['subdivision_id']);
		...
		$this->distributed_list   = $this->inm->division_list($div_data['division_id'],$div_data['subdivision_id'],$config1["per_page"], $page1);
	...
	function credits()
	{
		...
		if($result['credit'] == 'maitra'){
			$this->files = file_get_contents(base_url().'maitra-credits.txt', true);
		}else if($result['credit'] == 'icg'){
			$this->files = file_get_contents(base_url().'maitra-credits-icg.txt', true);
		}else{
			$this->files = file_get_contents(base_url().'maitra-credits-gnu.txt', true);
		}
	...
	{	 
		...
		if($_POST){			
			...
			$succmsg = $this->us->change_password($user_id,$old_password,$new_password);	
				if($succmsg==true)
				{
					$this->session->set_flashdata('message',"Password changed successfully");
					redirect('c=home&m=division_list'.$division_id.$subdivision_id);
				} else {
					$this->session->set_flashdata('message',"Failed to change password");
					redirect('c=home&m=division_list'.$division_id.$subdivision_id);
				}			
			}	 
	}	
	...
	function new_updates()
	{
		...
		$this->activities = $this->lg->fetch_logs_seperate_user();
		//$this->template_view('new_updates');
		...
		$this->load->model('inmails','inm');		
		$config['total_rows'] = $this->inm->new_updates_list_total_rows($v);
		$config['per_page'] = $this->settings['no_of_records'];		
		$config['base_url'] = site_url().'?c=home&m=new_updates'. $link;
		$config['uri_segment'] = 3;	
		$config['num_links'] = 10;
		//$config['use_page_numbers'] = TRUE;	
		$page = 0;
		...
		$this->distributed_list = $this->inm->new_updates_list($v,$config["per_page"], $page);	
		$this->template_view('new_updates');
	}
	
	function delete_single_log()
	{
		$this->load->model('settings','set');
		$result = $this->set->delete_single_log($_REQUEST['delete_mail_id']);
		return true;
	}
