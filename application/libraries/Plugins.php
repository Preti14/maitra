    ...
    */
    public function activate_plugin($name)
    {
        ...
        {            
			...
			$query = $this->_ci->db->query("SELECT plugin_system_name FROM plugins where plugin_system_name='helloworld'");
			
          //$this->_ci->db->select('plugin_system_name')->from('plugins')->where('plugin_system_name', $name); 
