...
<div class="box col-md-12">
    ...
    <table id="out_mail_list" class="table table-striped add-style table-bordered bootstrap-datatable datatable responsive">
    <thead>
    <tr>
	...
	foreach($outmail_list as $dist_list){
        ...
        <td class="no-print"><input type="checkbox" id="checkbox_<?=$dist_list['id'];?>" class="checkbox_option" name="checkbox" value="<?=$dist_list['id'];?>"></td>
            <td style="width:30px" class="no-print"><a href="<?=base_url();?>?c=outmail&m=outmailview&mailid=<?=$dist_list['id'];?>&stat=manageoutmail" data-toggle="tooltip" data-original-title="View"><?=$mailno_with_year;?></a></td>
	    <td class="only-print"><?=$mailno_with_year;?></td>            
            ...
            <td>
				<?=isset($dist_list['from'])?$dist_list['from']:'';?>
			</td>
			...
			if(isset($dist_list['subdivision']))
                ...
                <a title="Edit" href="<?=base_url();?>?c=outmail&m=outmailentry&mailid=<?=$dist_list['id'].$search;?>&ref=edit" class="link-color"><i class="glyphicon glyphicon-edit"></i></a> | <a href="javascript:void(0);" title="Delete" class="confirm red-icon" id="<?=$dist_list['id'];?>"><i class="glyphicon glyphicon-remove"></i></a>
...
$(document).ready(function() {
	...
	var mail_id = this.id;
	$("#"+mail_id).confirm({
		text: "Are you sure that you really want to delete this Outmail?",
	...
	$("#search_form").submit(function(e) {	
      	$("#search_form").attr("action", "<?php echo site_url()?>?c=admin&m=manage_outmails&mail_number="+$("#search_txt").val());
    });
